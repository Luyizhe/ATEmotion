...
def inference(test_loader, model, wav_or_dialogue, modal,savefile=None):
    confusion_Ypre = []
    confusion_Ylabel = []
    text_confusion_Ypre = []
    audio_confusion_Ypre = []
    ...
    print("Accuracy: %0.2f%%" % (100 * Best_Valid))
    print(acc_matrix)
    if modal == 'multi':
        print("Text Test Accuracy: %0.2f%%" % (100 * text_Best_Valid))
        print("Audio Test Accuracy: %0.2f%%" % (100 * audio_Best_Valid))
    if savefile != None:
        np.savez(savefile, matrix=acc_matrix, ACC=Best_Valid, text_matrix=text_acc_matrix, text_ACC=text_Best_Valid,
                 audio_matrix=audio_acc_matrix, audio_ACC=audio_Best_Valid)
